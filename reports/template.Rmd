...
promise.full.types <- get_full_promise_types(cutoff=.5)
...
```{r promise-freedom, echo=FALSE, include=FALSE}
promise.evaluation.freedom.histogram <- get_free_promise_evaluation_histogram()
promise.evaluation.cascade.histogram <- get_cascading_promises_evaluation_histogram(include.toplevel=FALSE)
```
...
``` {r promise-force-freedom-histogram, echo=FALSE, comment="", fig.width=5, fig.height=4.5}
par(mar=c(0,2,2,2))
pie(promise.evaluation.freedom.histogram$number, (promise.evaluation.freedom.histogram$free %>% make_labels), main="Free promise (forces)")
if(print_data) print(promise.evaluation.freedom.histogram %>% as.data.frame %>% transform(number=pp(number), percent=pp(percent)), row.names=FALSE)
```

</div>

``` {r promise-force-cascade-histogram, echo=FALSE, comment="", fig.width=5, fig.height=4.5}
histogram_with_labels <- 
  promise.evaluation.cascade.histogram %>% 
  mutate(no.of.forced.promises.inside=
           ifelse(no.of.forced.promises.inside==Inf, 
                  paste(">", cutoff, sep=""), 
                  as.character(no.of.forced.promises.inside)))
labels <- histogram_with_labels$no.of.forced.promises.inside
values <- histogram_with_labels$number
par(mar=c(2,2,2,2))

barplot(values, names.arg=labels, space=0, main="Cascading promise forces")
if(print_data) print(histogram_with_labels %>% as.data.frame %>% transform(number=pp(number), percent=pp(percent)), row.names=FALSE)
```
</div>
