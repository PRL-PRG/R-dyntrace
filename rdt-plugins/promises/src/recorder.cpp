...
recursion_type is_recursive(fn_id_t function) {
  for (vector<call_stack_elem_t>::reverse_iterator i =
           STATE(fun_stack).rbegin();
       i != STATE(fun_stack).rend(); ++i) {
    ...
    // inside a different function, but one that doesn't matter, recursion still
    // possible
  }
}
...
judge_promise_lifestyle(call_id_t from_call_id) {
  ...
  for (vector<call_stack_elem_t>::reverse_iterator i =
    ...
    if (cursor == from_call_id)
      if (effective_distance == 0) {
        if (actual_distance == 0) {
          ...
          return tuple<lifestyle_type, int, int>(
              lifestyle_type::LOCAL, effective_distance, actual_distance);
        }
      } else {
        if (effective_distance == 1) {
          return tuple<lifestyle_type, int, int>(
              lifestyle_type::IMMEDIATE_BRANCH_LOCAL, effective_distance,
              actual_distance);
        } else {
          return tuple<lifestyle_type, int, int>(lifestyle_type::BRANCH_LOCAL,
                                                 effective_distance,
                                                 actual_distance);
        }
      }
    ...
    if (cursor == 0) {
      return tuple<lifestyle_type, int, int>(
          lifestyle_type::ESCAPED, -1, -1); // reached root, parent must be in a
...
void get_full_type_inner(SEXP sexp, SEXP rho, full_sexp_type &result,
  ...
  if (type == sexp_type::BCODE) {
    ...
    //            SEXP underlying_expression = findVar(PRCODE(uncompiled_sexp),
    //            rho);
    //
  ...
  if (type == sexp_type::SYM) {
    ...
    /* FIXME - findVar can eval an expression. This can fire another hook,
       ...
       0x00000000004d4b3e in R_execClosure (call=0x94e1db0, newrho=0x94e1ec8,
       sysparent=0x1a05f80,
       ...
       rho=0x1a05f80, arglist=0x19d6b68, op=0x9534cb0) at eval.c:1612
       #1  0x00000000004d49e9 in Rf_applyClosure (call=0x94e1db0, op=0x9534cb0,
       arglist=0x19d6b68,
       ...
       at envir.c:1042
       #6  0x00000000004c1fb1 in Rf_findVar (symbol=0x54de8f8, rho=0x953dd50) at
       envir.c:1221
       ...
       result=std::vector of length 1, capacity 1 = {...}, visited=std::set with
       1 elements = {...})
       at
