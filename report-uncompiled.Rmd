...
<center>
``` {r lifestyles-pie, echo=FALSE, fig.width=7, fig.height=5.5, comment="", warning=FALSE}
...
promise.types <- get_promise_types(cutoff=1)

slices <- (promise.types %>% arrange(desc(number)))$number
labels <- (promise.types %>% arrange(desc(number)) %>% mutate(type = ifelse(percent>1, type, "")))$type
par(mar=c(0,2,2,2))
