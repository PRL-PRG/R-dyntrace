...
data <-
    ...
    rowwise() %>%
    mutate(begin = max(`0`, `1`, na.rm = TRUE),
           end   = min(`2`, max_trigger, na.rm = TRUE))

for(i in 1:length(data$begin)) {
    a <- data$begin[i] + 1
    b <- data$end[i] - 1
    if(a <= b) gc_data$`DISPENSABLE`[a : b] <- gc_data$`DISPENSABLE`[a : b] + 1;
}

# TODO - Replace NA with 0 to ensure correctness

# Remove last gc trigger data as this one never tells us how many are garbage collected.
# So this row will usually have NA.
gc_data <-
    gc_data %>%
    head(-1) %>%
...
graph <-
    ...
    ggtitle("Promise Garbage Collection",
            "Number of promises created before, looked up before and collected at each GC trigger") +
    xlab("GC Trigger") +
...
```{r more_meaningful_graph, echo  = FALSE}
# this is a dirty hack to get ggplot2 to make the graph.
# Apparently, it is not possible to mix and match stacked and dodged bar charts in ggplot2.
# Hence, I modify the data to get the bar lengths I want.
partial_data <-
    gc_data %>%
