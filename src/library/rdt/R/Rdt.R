...
trace.promises.compiled.db <- function(expression, tracer="promises", output="DB", path="trace.sqlite", format="psql", pretty.print=FALSE, overwrite=TRUE, synthetic.call.id=TRUE)
    Rdt(expression, tracer=tracer, output=output, path=path, format=format, pretty.print=pretty.print, synthetic.call.id=synthetic.call.id, overwrite=overwrite)

trace.promises.uncompiled.db <- function(expression, tracer="promises", output="DB", path="trace.sqlite", format="sql", pretty.print=FALSE, overwrite=TRUE, synthetic.call.id=TRUE)
    Rdt(expression, tracer=tracer, output=output, path=path, format=format, pretty.print=pretty.print, synthetic.call.id=synthetic.call.id, overwrite=overwrite)
